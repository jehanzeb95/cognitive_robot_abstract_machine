...
class QPSolverProxsuite(QPSolver):
    ...
    def __init__(
        ...
        self.num_non_slack_variables = (
            self.num_free_variable_constraints - self.num_slack_variables
        )
        ...
        free_symbols = set(weights.free_variables())
        free_symbols.update(combined_E.free_variables())
        free_symbols.update(combined_A.free_variables())
        free_symbols.update(lb.free_variables())
        free_symbols.update(ub.free_variables())
        free_symbols.update(bE.free_variables())
        free_symbols.update(lbA.free_variables())
        free_symbols.update(ubA.free_variables())
        ...
        self.combined_vector_f = cas.CompiledFunctionWithViews(
            expressions=[lb, lbA, weights, g, bE, ub, ubA],
            parameters=free_symbols,
            additional_views=[slice(0, b_bA_len), slice(-b_bA_len, None)],
        )
    ...
    def solver_call_explicit_interface(
        self, H, g, lb, ub, E, bE, A, lbA, ubA
    ) -> np.ndarray:
        A2 = np.eye(len(ub))
        if len(E) > 0:
            A2 = np.vstack((A2, E))
    ...
    def evaluate_functions(self, substitutions: np.ndarray):
        (
            ...
            self.lb_lbA,
            self.ub_ubA,
        ) = self.combined_vector_f.fast_call(substitutions)
        self.E = self.E_f.fast_call(substitutions)
        self.A = self.A_f.fast_call(substitutions)

    def update_filters(self):
        self.weight_filter = self.weights != 0
        self.weight_filter[: -self.num_slack_variables] = (
            True  # filter only slack variables with weight 0
        )
    ...
    def apply_filters(self):
        ...
        if self.compute_nI_I:
            ...
            self.Ai = self._direct_limit_model(
                self.weights.shape[0], self.Ai_inf_filter
            )
    ...
    def solver_call(
        ...
        if qp.results.info.status.value != 0:
            ...
            raise InfeasibleException(
                f"Failed to solve qp: {qp.results.info.status.name}"
            )
        return qp.results.x
    ...
    def relaxed_problem_data_to_qp_format(
        self,
        ...
        if self.retries_with_relaxed_constraints <= 0:
            raise HardConstraintsViolatedException(
                "Out of retries with relaxed hard constraints."
            )
        lb_filter, lbA_relaxed, ub_filter, ubA_relaxed, relaxed_qp_data = (
            self.compute_violated_constraints(
                self.lb[self.weight_filter], self.ub[self.weight_filter]
            )
        )
    ...
    def compute_violated_constraints(
        self, filtered_lb: np.ndarray, filtered_ub: np.ndarray
    ):
        lb_relaxed = filtered_lb.copy()
        ub_relaxed = filtered_ub.copy()
        slack_finite_filter = self.weight_filter[
            -(self.num_eq_slack_variables + self.num_neq_slack_variables) :
        ]
        num_constraints_not_filtered = np.where(self.slack_part[slack_finite_filter])[
            0
        ].shape[0]
        ...
        try:
            ...
            xdot_full = self.solver_call(
                H=H, g=g, E=E, b=bE, A=A, lbA=lbA, ubA=ubA, lb=lb_relaxed, ub=ub_relaxed
            )
        ...
        lb_relaxed[
            lower_violations[self.b_finite_filter[self.b_zero_filter]]
        ] -= self.retry_added_slack
        ub_relaxed[
            upper_violations[self.b_finite_filter[self.b_zero_filter]]
        ] += self.retry_added_slack
        ...
        return (
            ...
            upper_violations,
            ubA_relaxed,
            (H, g, A, all_lbA_relaxed, all_ubA_relaxed),
        )
